...
+ (NSPoint)		colinearPointForPoint:(NSPoint) p centrePoint:(NSPoint) q
{
	...
	float r = hypotf( p.x - q.x, p.y - q.y );
	return [self colinearPointForPoint:p centrePoint:q radius:r];
}
...
+ (int)			point:(NSPoint) p inNSPointArray:(NSPoint*) array count:(int) count tolerance:(float) t
{
	...
	// the point in the array "hit" by p, or -1 if not hit.
...
+ (void)				colineariseVertex:(NSPoint[3]) inPoints cpA:(NSPoint*) outCPA cpB:(NSPoint*) outCPB
{
	...
	float r1 = hypotf( inPoints[0].x - inPoints[1].x, inPoints[0].y - inPoints[1].y );//LineLength( inPoints[0], inPoints[1] );
	float r2 = hypotf( inPoints[2].x - inPoints[1].x, inPoints[2].y - inPoints[1].y );//LineLength( inPoints[1], inPoints[2] );
...
- (int)					elementHitByPoint:(NSPoint) p tolerance:(float) tol tValue:(float*) t nearestPoint:(NSPoint*) npp
{
	...
	if ( NSPointInRect( p, bb ))
	{
		...
		//NSLog(@"point in element bbox, elem = %d", elem );
		...
		if ( elem > 0 )
		{
			...
			else if ( etype != NSMoveToBezierPathElement )
			{
				...
				np = NearestPointOnLine( p, ap[0], lp[0] );
				tt = RelPoint( np, ap[0], lp[0] );
			}	
...
- (int)					elementBoundsContainsPoint:(NSPoint) p tolerance:(float) tol
{
	...
	for( i = 0; i < m; ++i )
	{
		...
		if ( NSPointInRect( p, NSInsetRect( bb, -tol, -tol )))
			return i;
	}
...
- (NSRect)				boundingBoxForElement:(int) elementIndex
{
	...
	if ( lm != NSMoveToBezierPathElement )
	{
		if ( lm == NSCurveToBezierPathElement )
		{
			...
			bb.origin.x = minx;
			bb.origin.y = miny;
			bb.size.width = maxx - minx;
			bb.size.height = maxy - miny;
		}
		else if ( lm == NSClosePathBezierPathElement )
		{
			int fe = [self subpathStartingElementForElement:elementIndex];
			[self elementAtIndex:fe associatedPoints:pp];
			pm = [self elementAtIndex:elementIndex - 1 associatedPoints:ap];
			
			if( pm == NSCurveToBezierPathElement )
				bb = NSRectFromTwoPoints( ap[2], pp[0] );
			else
