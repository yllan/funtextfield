...
static const float deg60 = 1.0471975512;
static const float sin60 = 0.8660254038;
...
+ (NSBezierPath*)		bezierPathWithStandardChainLinkFromPoint:(NSPoint) a toPoint:(NSPoint) b
{
	...
	float		slope = atan2f( b.y - a.y, b.x - a.x );
...
+ (NSBezierPath*)		bezierPathWithSprocketPitch:(float) pitch numberOfTeeth:(int) teeth
{
	...
	float toothAngle = pi / teeth;
	float radius = pitch / ( 2 * sinf( toothAngle ));
	float rollerRadius = pitch / 3.6f;
	float toothRadius = pitch - rollerRadius;
	...
	float taDegrees = ( toothAngle * 180.0 ) / pi;
	
	[tooth appendBezierPathWithArcWithCenter:rp1 radius:rollerRadius startAngle:180 + taDegrees endAngle:270 clockwise:NO];
	
	// flank of tooth follows the larger radius until it reaches the halfway point. The x4 here stops it slightly short so that
	// the top edge of the tooth is flattened off a little
	
	float endAngle = ( cosf( pitch / ( 3 * toothRadius )) * 180.0 ) / pi;
...
+ (NSBezierPath*)		bezierPathWithThreadedBarOfLength:(float) length diameter:(float) dia threadPitch:(float) pitch options:(unsigned) options
{
	...
	NSPoint			p;
	NSBezierPath*	path = [self bezierPath];
	float			xIncrement = pitch * 0.5f;
	float			yIncrement = pitch * sin60;
	float			phase = -1;
									...
									options:(unsigned) options
{
	...
	float threadLength = length - hheight - shank;
