...
- (void)				setUnderlined:(int) uval
{
	if(![self locked])
	{
		[self changeTextAttribute:NSUnderlineStyleAttributeName  toValue:[NSNumber numberWithInt:uval]];
...
- (int)					underlined
{
	return [[[self textAttributes] objectForKey:NSUnderlineStyleAttributeName] intValue];
}
...
- (DKStyle*)			drawingStyleFromTextAttributes
{
	...
	float sw = [[[self textAttributes] objectForKey:NSStrokeWidthAttributeName] floatValue];
